...
class _ShowMarkedStudentsState extends State<ShowMarkedStudents> {
  ...
  var Result;
  void getdateinfo() async{//gets info from given date
    var result=await getSpecificCourseDateInfo("4-11-2020", "MIN-106");
    setState(() {
      Result=result;
    });
  }
  void getinfo() async{//get info for a course
    var enroll_temp=[];
    var result=[];
    result= await getSpecificCourseInfo("MIN-106");
    for(int i=0; i<result.length; i++){
      enroll_temp.add(result[i].get("Date"));
    }
    setState((){
      Enrollment=enroll_temp;
    });
  }
  @override
  Widget build(BuildContext context) {
