...
class showAttendanceUploadFailure{
  // ignore: non_constant_identifier_names
  static Future<DialogAction> ConfirmDialog(
      BuildContext context,
      String title,
    ...
    final action = await showDialog(
        ...
        builder: (BuildContext context){
          return AlertDialog(
              ...
              title: Icon(Icons.close, size: 40, color: Colors.red,),
              content: Text("Oops! The system could not register your attendance for " + courseName + "\nPlease Try taking attendance again. :(", style: TextStyle(fontWeight: FontWeight.bold), textAlign: TextAlign.center,),
              actions: <Widget>[
                RaisedButton(
                    onPressed: () => Navigator.of(context).pop(DialogAction.Confirm),
                    color: PrimaryColor,
